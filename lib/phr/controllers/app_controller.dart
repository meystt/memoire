...
class AppController extends GetxController {
  ...
  int glucoseCalculation({ int when,  int unit}) {
    var result = 0;
    // calculate fasting
    if (when == 0) {
      if ((unit >= 80) && (unit <= 100)) {
  ...
  setProfile({ String name,  String photo}) {
  ...
  // add user profile
  addProfile({ String name,  String photo}) async {
    final box = await Hive.openBox<Settings>('Settings');
  ...
  deleteBMI({ String key}) async {
    final box = await Hive.openBox<Bmi>('Bmi');
    box.delete(key);
    update();
  }

  deleteBloodPressure({ String key}) async {
    final box = await Hive.openBox<BloodPressure>('BloodPressure');
    box.delete(key);
    update();
  }

  deleteGluecose({ String key}) async {
    final box = await Hive.openBox<Glucose>('Glucose');
    box.delete(key);
    update();
  }

  getDataOnly({ List<dynamic> box,  int total}) {
